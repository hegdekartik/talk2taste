...
If an image is provided, ensure your recipe name and context match the provided image.`;
            ...
            if (imageBase64) {
                contents[0].parts.push({ inline_data: { mime_type: imageFile.type, data: imageBase64 } });
            }
            ...
            // Render the raw output first using Marked (or just fallback if format is too variable)
            rawOutput.innerHTML = marked.parse(textOutput);
            rawOutput.classList.remove('hidden');
            
            // Hide the structured components for now, as LLM output might not always be perfectly parsable
            // But we keep the UI components ready for future structural improvements.
